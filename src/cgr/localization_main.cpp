...
* \author  Joydeep Biswas, (C) 2010. Original implementation.
* \author  Fernando J. Iglesias Garcia, (C) 2013. Distributed with GraphLab.
*/
//========================================================================
...
void LoadParameters()
{
  ...
  ConfigReader config(ros::package::getPath("cgr_graphlab").append("/").c_str());
...
//     transformListener.lookupTransform("base_link", "camera_link", ros::Time(0), baseLinkToKinect);
  ...
  if(debugLevel>0){
    ...
    for(unsigned int i=0; i<completePointCloud.size(); i++){
